...
const CEO_SUBAGENT = {
  ...
  reads: ["Tethr Drive", "memory", "the existing agent roster"],
  steps: [
    ...
    "If a priority needs a capability no existing agent covers, propose ONE agent for it with propose_agent (never a duplicate)",
    "Save the brief to the Drive",
  ],
  output: "A short, dated priorities brief in the Drive (plus any agent proposals staged for approval).",
  guardrails: [
    "Internal-only — sets direction, never publishes or spends",
    "Delegate by naming owners; don't do the specialists' work",
    "Propose an agent ONLY when the org genuinely lacks a needed capability — one per gap, never a duplicate of an existing agent; each proposal waits for human approval (or applies if you're on auto), and the new agent starts paused",
