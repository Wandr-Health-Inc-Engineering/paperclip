...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Tethr", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Tethr",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Tethr");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Tethr");
    expect(buildDocumentTitle([], null)).toBe("Tethr");
  });
});
